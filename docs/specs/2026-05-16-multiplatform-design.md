...
hibiki/
├── packages/
│   ├── hibiki_core/            # 纯 Dart：模型、数据库(Drift)、EPUB/字幕解析、i18n
...
包间依赖（无循环）：
  hibiki_core          不依赖其他 hibiki 包（最底层：模型、数据库、解析器、i18n、Language 抽象类）
  hibiki_dictionary  -> hibiki_core（DictEntry 模型、数据库 DAO、Language 抽象类；包含 language implementations）
...
> **耦合说明：** 当前 `language/` 模块深度依赖 `dictionary/`（lookup/deinflection）、`models/app_model.dart`（3,456 行 God object）和 `utils/`（含 UI widget）。Phase 0 的首要任务是解耦：提取 `Language` 抽象接口到 `hibiki_core`（无词典依赖），language 具体实现到 `hibiki_dictionary`，并通过依赖注入替代对 AppModel 的直接引用。
